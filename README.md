...
| 이미지 thumbnail | 4초 표시, 최대 220×140, 둥근 모서리, 2px 테두리, 클릭 시 기본 이미지 뷰어로 열기 |
...
ImageMagick / xclip 프로세스가 필요하지 않습니다. 클립보드 모드에서는 캡처만으로 이미지 파일을
만들지 않으며, thumbnail을 클릭해 기본 뷰어로 열 때만 임시 PNG를 만듭니다.
